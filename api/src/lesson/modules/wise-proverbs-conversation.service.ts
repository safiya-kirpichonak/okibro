...
import * as proverbs from '../../common/data/proverbs.json';
...
@Injectable()
export class WiseProverbsConversationService {
  constructor(
  ...
  async startFirstPart(
    ...
    let { content } = await this.prisma.prompt.findFirst({
      where: { code: 'wise-proverbs-conversation-1' },
    });
    const randomIndex = GeneralService.generateRandomIndex(proverbs.length);
    content = content.replace('DYNAMIC_PART', proverbs[randomIndex]);
    ...
    let { proverbs: proverbsSettings, startTime } = lesson.settings;
    if (proverbsSettings) {
      proverbsSettings.push(proverbs[randomIndex]);
    } else {
      proverbsSettings = [proverbs[randomIndex]];
    }
  ...
  async startSecondPart(
    ...
    let { proverbs: proverbsSettings, startTime } = lesson.settings;
    if (proverbsSettings.length === 0) proverbsSettings = [...proverbs.slice(0, 100)];

    let { content } = await this.prisma.prompt.findFirst({
      where: { code: 'wise-proverbs-conversation-2' },
    });
    const randomIndex = GeneralService.generateRandomIndex(
      proverbsSettings.length,
    );
    content = content.replace('DYNAMIC_PART', proverbsSettings[randomIndex]);
    proverbsSettings.splice(randomIndex, 1);
    const messages = [{ role: 'user', content }];
  ...
  async speakSecondPart(
    ...
    const content = `This is my answer, and analyze only this: "${userText}".  
                     Next explain and give example for: ${proverbsSettings[randomIndex]}`;
    proverbsSettings.splice(randomIndex, 1);
